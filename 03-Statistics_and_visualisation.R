...
controlledDrills <- reduce(df, rbind) %>%
  ...
  mutate(
    truth = fct_recode(truth, "Linear running" = "Running", "Non-linear running" = "Dynamic"),
...
drills <- c(
  "Walking", "LinearRun10", "LinearRun15", "LinearRun16",
...
textSize <- 8
ggplot() +
  scatterpie::geom_scatterpie(aes(x = x, y = y, group = athlete),
    data = plotData, alpha = 0.9,
  ...
  scale_fill_manual(
    ...
    values = c(
      "No Locomotion" = "#b2df8a", "Walking" = "#a6cee3",
