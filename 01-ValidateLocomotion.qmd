...
```{r}
#binding all the respective locomotion types across data collections.
...
```{r}
#Table for predicted percent distribution for each category.
...
percentDisagreement <- 
  data.frame(
    ...
    NoLocomotion = c(0.3830369, 0L, 0L, 0L, 0L, 0.1571092, 0L, 0L),
    SlowLocomotion = c(99.2065663, 0L, 0L, 0L, 0L, 6.2843676, 0L, 0L),
    LinearLocomotion = c(0, 95.486111, 88.31169, 81.00358, 74.01434, 17.8318932, 1.699717, 2.017291),
    NonLinearLocomotion = c(0.4103967, 4.513889, 11.68831, 18.99642, 25.98566, 75.7266300, 98.300283, 97.982709)
    
  
)

data.table::fwrite(percentDisagreement, "Data/ProcessedData/percentDisagreement.csv")
...
textSize = 8
ggplot() +
  scatterpie::geom_scatterpie(aes(x = x, y = y, group = athlete), data = t, alpha = 0.9,
                              cols = c("dynamic","running","walking","no_locomotion"), 
                              pie_scale = 0.5) +
  geom_vline(xintercept = c(0,1,2,3,4,5,6,7,8), linetype = "dashed", color = "#838383", size = 2, alpha = 0.75) + 
